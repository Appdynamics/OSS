...
/**
 * This filter tries to detect whether a request URL needs to be decorated with
 * a ;jsessionid token in the event that the client does not support cookies.
 * <p>
 * We don't do this for ColdFusion however as they handle this in their side of
 * the Flash Remoting code.
 * </p>
 *
    ...
    public void invoke(final ActionContext context) throws IOException
    {
        ...
        try
        {
            ...
            if (reqURL != null)
            {
                ...
                {
                    context.getResponseMessage().addHeader(new MessageHeader(MessageIOConstants.URL_APPEND_HEADER, false, sessionSuffix));
                }
            }
        }
        catch (Throwable t)
