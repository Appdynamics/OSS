...
//                ? new TraceClassVisitor(cw, new PrintWriter(System.out)) : cw;
    ...
    private static void generateGetSerializableObjectFactoryMethod(ClassVisitor classVisitor,
        ...
        MethodVisitor cv = classVisitor.visitMethod(ACC_PUBLIC, "getSerializableObjectFactory", "()L" + SerializableObjectFactory.class.getName().replace('.', '/') + ";" , null, new String[] { "java/io/IOException" });
        cv.visitVarInsn(ALOAD, 0);
        ...
        cv.visitMethodInsn(INVOKEINTERFACE, IndirectlySerializable.class.getName().replace('.', '/'), "getSerializableObjectFactory", "()L" + SerializableObjectFactory.class.getName().replace('.', '/') + ";");
