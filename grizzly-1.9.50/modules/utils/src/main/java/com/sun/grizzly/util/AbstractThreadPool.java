...
public abstract class AbstractThreadPool extends AbstractExecutorService
	...
	protected AtomicInteger workersExecutingTasks = new AtomicInteger(0);
    ...
    protected void onTaskQueued(Runnable task) {
      if (probe != null){
		...
		if (task instanceof TaskExecutionMonitor)
		{
			((TaskExecutionMonitor) task).monitorTaskQueued();
		}

		if (LOGGER.isLoggable(Level.FINEST))
		{
			LOGGER.log(Level.FINEST, "workersExecutingTasks " + workersExecutingTasks.get());
		}
    }
    ...
    protected void onTaskDequeued(Runnable task) {
      if (probe != null){
		...
		if (task instanceof TaskExecutionMonitor)
		{
			((TaskExecutionMonitor) task).monitorTaskDequeued(getName(), getQueueSize(), getMaxQueuedTasksCount());
		}
    }
        ...
        protected void doWork(){
            ...
            while(true) {
                try {                    
                    ...
                    try {
						workersExecutingTasks.incrementAndGet();
						...
						workersExecutingTasks.decrementAndGet();

                        if (r instanceof TaskExecutionMonitor)
                        {
                            ((TaskExecutionMonitor) r).monitorTaskExecutionTime(System.currentTimeMillis() - startTS, getName());
                        }
                    }
                } catch (Throwable throwable) {
                }
